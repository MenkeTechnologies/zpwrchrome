...
//
// The Rust port of browserpass-native (zpwrchrome-host) does NOT do
...
// the previous Rust implementation in zpwrchrome-host/src/pass.rs
...
// When no explicit URL field is present, use the first path segment (the
// parent directory of the basename) verbatim as the URL — no `https://`
// prepend, no host validation. The user's store convention is
// `<host-or-brand>/<username>` and they want the URL row populated
// literally for every entry. Single-segment entries use that segment.
//   `adobe.com/jmenke@wccnet.edu` → url = "adobe.com"
//   `amcrest/admin`               → url = "amcrest"
//   `aws/prod/root`               → url = "aws"
//   `subdomain.example.com.gpg`   → url = "subdomain.example.com"
export function fallbackUrlFromPath(parsed, entryPath) {
  if (parsed.url) return parsed;
  const stripped = String(entryPath || "").replace(/\.gpg$/, "");
  const segs = stripped.split("/").filter(Boolean);
  if (!segs.length) return parsed;
  parsed.url = segs[0];
  return parsed;
}
