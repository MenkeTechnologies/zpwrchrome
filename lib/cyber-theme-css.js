...
export function buildThemeCss(opts) {
  ...
  // o.palette (camelCase THEME shape, e.g. from color-schemes.js themeFor)
  // lets any of the 8 schemes drive the recolor; defaults to Cyberpunk.
  const t = o.palette || THEME;
  const parts = [];
  ...
  if (forceMono) {
    ...
    parts.push(`
      *:not(code):not(pre):not(kbd):not(samp):not(tt):not(i):not(svg):not(svg *):not([class*="icon"]):not([class*="Icon"]):not([class*="fa-"]):not([class*="material-icons"]):not([class*="material-symbols"]):not([class*="lucide"]):not([class*="phosphor"]):not([class*="ph-"]):not([class*="glyphicon"]):not([data-icon]):not([data-lucide]):not([data-cds="Icon"]):not([data-radix-icon]) {
