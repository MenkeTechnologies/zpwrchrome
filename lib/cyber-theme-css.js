...
//   darkMode:  boolean  (default false)  — Dark-Reader-style CSS filter
//                                          inversion on <html>, re-inverting
//                                          images/video/canvas so they keep
//                                          their original colors. Works on
//                                          any page without needing selectors
//                                          to match (Amazon, gmail, etc.)
//
...
export function buildThemeCss(opts) {
  ...
  // ─── Dark-mode filter (applied first so subsequent rules layer on
  //     top of the inverted base). Uses the same technique as Dark
  //     Reader: invert the whole document, then re-invert media so
  //     photos/videos/canvases keep their original colors. invert(0.92)
  //     instead of 1.0 produces a less harsh look; hue-rotate(180deg)
  //     restores hue after inversion so blue stays blue.
  if (darkMode) {
    parts.push(`
      html {
        /* Paint html WHITE so the filter inverts it to near-black —
           anything we set on html is itself inverted, so a dark color
           here would paint LIGHT in the viewport gutter around fixed-
           width layouts (Amazon, Gmail). White → ~rgb(20,20,20). */
        background-color: white !important;
        filter: invert(0.92) hue-rotate(180deg) contrast(0.95) !important;
      }
      /* Re-invert only ACTUAL bitmap/vector media so photos keep their
         original colors. CSS gradients are NOT included — they're a
         re-color of the page, not a photo, and they should follow the
         page inversion. Catching gradient inline-styles was the bug
         that left Amazon's order cards white. */
      ...
      /* Cancel the global filter on fixed/sticky overlays so they don't
         double-invert relative to the host page. */
  ...
  if (forceMono) {
    parts.push(`
      *:not(code):not(pre):not(kbd):not(samp):not(tt) {
