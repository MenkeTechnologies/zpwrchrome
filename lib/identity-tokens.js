...
export const TOKEN_SYNONYMS = Object.freeze({
  ...
  "address-line3": ["address-line3", "addressline3", "address3", "addr3",
                    "street3", "address_3"],
  "street-address":["street-address", "streetaddress", "street"],
...
export function recognizeField(spec) {
  ...
  if (ac) {
    ...
    // off / on / new-password / current-password are explicit opt-outs
    if (ac === "off" || ac === "current-password" || ac === "new-password") {
      return null;
    }
  }
...
// alias chains (cc-exp from cc-exp-month + cc-exp-year, name from
// given+family, given/family-name split from name, etc.). Returns null
