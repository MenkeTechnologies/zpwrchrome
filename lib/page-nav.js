...
// Apply the chosen color scheme to this dashboard page. Every dashboard
// imports page-nav, so this single import recolors them all.
import "./ui-scheme.js";

const LINKS = [
  { key: "dashboard",   label: "dashboard",   href: "/scripts-manager/dashboard.html" },
  ...
  { key: "postcommands",label: "post-cmd",    href: "/scripts-manager/dl-postcommands.html" },
  { key: "userscripts", label: "userscripts", href: "/scripts-manager/manager.html" },
  { key: "pass",        label: "pass",        href: "/scripts-manager/pass.html" },
  { key: "find",        label: "find in tabs",href: "/scripts-manager/find-all.html" },
  { key: "ua",          label: "ua switcher", href: "/scripts-manager/ua-switcher.html" },
  { key: "modheader",   label: "modheader",   href: "/scripts-manager/modheader.html" },
  { key: "theme",       label: "cyber theme", href: "/scripts-manager/theme-injector.html" },
  { key: "lights",      label: "lights-off",  href: "/scripts-manager/lights-off.html" },
  { key: "reader",      label: "reader mode", href: "/scripts-manager/reader-mode.html" },
  { key: "host",        label: "host console", href: "/scripts-manager/host.html" },
...
function pageKeyFromPath() {
  ...
  if (p.endsWith("dl-postcommands.html")) return "postcommands";
  if (p.endsWith("host.html"))          return "host";
  ...
  if (p.endsWith("find-all.html"))      return "find";
  if (p.endsWith("ua-switcher.html"))   return "ua";
  if (p.endsWith("modheader.html"))     return "modheader";
  if (p.endsWith("theme-injector.html")) return "theme";
  if (p.endsWith("lights-off.html"))     return "lights";
  if (p.endsWith("reader-mode.html"))    return "reader";
  return null;
}
...
// zwire command palette (⌘K). Load the zgui-core bundle + the SHARED item source
// (palette-cmds.js / cmd-defaults.js, identical to the HUD + New Tab) + our glue
// (zpc-palette.js) as classic scripts, in order. The palette is opened only when
// hud-internal's ⌘K router relays to us (see lib/zpc-palette.js) — inside zwire it
// is the same palette as everywhere else; standalone it simply never opens.
// `async = false` forces ordered execution of dynamically-inserted scripts.
function loadPalette() {
  if (document.getElementById("zpc-palette-loaded")) return;
  const marker = document.createElement("meta");
  marker.id = "zpc-palette-loaded";
  document.head.appendChild(marker);
  const files = [
    "lib/zgui/util.js",
    "lib/zgui/fzf.js",
    "lib/zgui/command-palette.js",
    "lib/palette-cmds.js",
    "lib/cmd-defaults.js",
    "lib/zpc-palette.js",
  ];
...
try { loadPalette(); } catch (e) { /* palette is best-effort; nav still works */ }
