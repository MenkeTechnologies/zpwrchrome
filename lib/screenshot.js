...
/**
 * Capture the full scrollable area of the given tab and return the raw
 * PNG bytes + a sensible default filename. Caller is responsible for
 * writing them to disk (background.js routes through bpSend → dl.writeFile
 * directly because chrome.runtime.sendMessage from a service worker
 * doesn't deliver back to the same service worker).
 *
 * @param {chrome.tabs.Tab} [tab] — defaults to the active tab
 * @returns {Promise<{blob: Blob, filename: string, host: string}>}
 */
export async function screenshotFullPage(tab) {
  ...
  return { blob, filename, host };
}

/** Convert a Blob to a base64 string. Chunked so big PNGs don't blow the call stack. */
