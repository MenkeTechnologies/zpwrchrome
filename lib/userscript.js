...
// Tampermonkey-friendly expansion. Chrome's match-pattern spec says
// `https://example.com/*` matches ONLY the apex — not www.example.com,
// not subdomains. New users write `@match https://amazon.com/*` and
// can't figure out why nothing fires on www.amazon.com.
//
// expandMatchPatterns takes a list of @match values and, for every bare
// host (no leading `*.`), adds a `*.host` variant. Chrome's `*.host`
// form matches BOTH the apex and all subdomains, so the original is
// effectively a subset — but we keep both to make the intent explicit.
//
// Bare IPs / single-label hosts (`localhost`) are left alone.
export function expandMatchPatterns(patterns) {
  ...
  for (const p of patterns) {
    ...
    // Already has a wildcard somewhere in the host, or it's a single
    // label (localhost, ip), or no dot — leave alone.
    if (host === "*" || host.startsWith("*") || !host.includes(".")) continue;
    if (/^[\d.]+$/.test(host)) continue; // raw IP
