...
export function mruStep(mru, currentId, delta) {
  ...
  // JS's % keeps the sign of the dividend; double-mod normalizes any delta
  // (including |delta| ≥ mru.length) into a valid [0, length) index.
  const len = mru.length;
  return mru[((idx + delta) % len + len) % len];
}
