...
export function extractCslFromPage() {
  ...
  // Organisation names must stay single-field CSL `literal` names. Splitting one into
  // given/family takes the LAST word as the surname, so "National Research Council (US)
  // Subcommittee on Laboratory Animal Nutrition" becomes family "Nutrition" + given
  // "National Research Council (US) Subcommittee on Laboratory Animal", which any
  // author-date style then initialises into "Nutrition, N. R. C. (. S. o. L. A."
  const ORG_WORD = new RegExp(
    "\\b(universit(?:y|e|ies)|institut(?:e|ion)?|college|school|department|dept|faculty|" +
      "laborator(?:y|ies)|council|committee|subcommittee|commission|board|panel|bureau|" +
      "office|agency|administration|ministry|academy|society|association|federation|" +
      "foundation|trust|consortium|collaboration|network|group|team|programme|program|" +
      "project|initiative|centre|center|hospital|clinic|museum|library|archives|press|" +
      "publishers?|inc|llc|ltd|gmbh|corp(?:oration)?|organi[sz]ation|survey|authority)\\b",
    "i"
  );
  // Parenthesised qualifiers ("(US)", "(WHO)") never occur in a personal name, and a long
  // name strung together with of/for/on/and/the is a body, not a person.
  const isOrganisation = (s) =>
    ORG_WORD.test(s) ||
    /[()]/.test(s) ||
    (/\b(of|for|on|and|the)\b/i.test(s) && s.split(/\s+/).length >= 4);

  // "Last, First" or "First Last" or institutional → a CSL name object.
  const parseName = (raw) => {
    raw = String(raw || "").trim();
    if (!raw) return null;
    if (isOrganisation(raw)) return { literal: raw };
    if (raw.includes(",")) {
  ...
  // Publishers routinely emit the same author in both citation_author and DC.creator, which
  // would otherwise list every author twice in the bibliography.
  const dedupeNames = (list) => {
    const seen = Object.create(null);
    return list.filter((n) => {
      const key = (n.literal || `${n.family}|${n.given}`).toLowerCase().replace(/\s+/g, " ");
  ...
  const authors = dedupeNames(
    metas("citation_author").concat(metas("dc.creator")).map(parseName).filter(Boolean)
  );
  ...
  if (!out.type || !out.author) {
    try {
      ...
      for (const b of blocks) {
        ...
        // Springer-Nature (and others) wrap the article in a WebPage:
        // {"@type":"WebPage","mainEntity":{"@type":"ScholarlyArticle",…}}. Walking only the
        // top level or @graph skips those pages' JSON-LD entirely.
        const nodes = [];
        const collect = (n, depth) => {
          if (!n || typeof n !== "object" || depth > 3) return;
          nodes.push(n);
          if (n.mainEntity) collect(n.mainEntity, depth + 1);
          if (Array.isArray(n["@graph"])) n["@graph"].forEach((g) => collect(g, depth + 1));
        };
        (Array.isArray(data) ? data : [data]).forEach((n) => collect(n, 0));
        for (const node of nodes) {
          ...
          if (!out.author && node.author) {
            ...
            const mapped = dedupeNames(
              al.map((a) => parseName(typeof a === "string" ? a : a && a.name)).filter(Boolean)
            );
