...
- **Profile + credit-card autofill from \`pass\`** — two new commands: \`pass-fill-profile\` fills name / address / email / phone / etc. on the active tab from \`profile/<name>\` entries; \`pass-fill-cc\` fills card-number / exp / csc / cardholder from \`creditcard/<name>\` entries. Entry keys can be either the WHATWG HTML autocomplete tokens (\`given-name\`, \`street-address\`, \`postal-code\`, \`cc-exp-month\`, …) **or** friendly synonyms (\`first-name\`, \`address\`, \`city\`, \`state\`, \`zipcode\`, \`cvv\`, …) — both resolve to the same field. Field recognition: \`<input autocomplete=…>\` wins outright (composite forms like \`shipping street-address\` supported), then an ordered regex ruleset (\`FIELD_RULE_SOURCES\`, ported from the Firefox HeuristicsRegExp + Chromium autofill patterns — the de-facto spec for real-world field-name spellings) matched across name+id+label+placeholder, then \`<input type="email|tel">\`. Names are canonicalized first (camelCase split, then every non-alphanumeric run — spaces, \`_\`, \`.\`, \`[\`, \`]\` — folded to \`-\`), so nested framework names resolve too: \`address.line1\`, \`billing[postcode]\`, \`checkout[shipping_address][zip]\`, \`billing_address_1\`, \`addressLine1\` all match. Rule order is precedence (specific beats general): cc-exp-month/year before cc-exp, the leaf address tokens (city/state/zip/country) before the catch-all street-address so a nested ZIP field is never swallowed as an address. Collision guards ride Chrome's lookbehind/lookahead: state ≠ "united states", \`\\bcity\\b\` ≠ "capacity"/"electricity", county → state, \`unit(?!e)\`, email-address → email. Alias chains backfill: \`cc-exp\` ← month/year, \`name\` ← given+family, given/family ← split of \`name\`, \`street-address\` ← line1+line2+line3. **Widget-aware fill** drives more than plain \`<input>\`s: native \`<select>\` via tolerant option-matching (value or visible text), custom dropdowns / comboboxes (react-select, MUI, Ant, Downshift, Headless UI, Radix, react-aria — anything exposing \`role="combobox"\` / \`aria-haspopup="listbox"\` + \`role="option"\`) by opening, type-to-filter, polling for the option (handles async-loaded menus like geocoded city fields) and clicking it, and \`intl-tel-input\` phone fields (a full \`+E.164\` number auto-selects the country). \`country: US\` / \`state: CA\` resolve to the dropdown's display name ("United States" / "California") via embedded ISO-3166 + USPS + CA-province tables (\`lib/geo-data.js\`). \`autocomplete="off"\` no longer suppresses recognition (custom widgets set it on their inner inputs). The field scan (and \`pass-fill\`'s login scan) pierces **open shadow roots** — web-component design systems (SmartRecruiters spl-input, Salesforce LWC, Vaadin, …) render the real \`<input>\` inside a shadow root where \`document.querySelectorAll\` never looks; the injectors recursively descend every open root (labels resolve via \`getRootNode()\` too), so those forms fill like plain HTML. Card number / CVC / expiry rendered in a processor's cross-origin iframe (Stripe Elements, Braintree, Adyen, Square, Checkout.com, Recurly, Spreedly, Shopify) are physically unfillable by any extension — those are detected and reported so you know to enter the card manually rather than left silently blank. Multi-entry stores get an in-tab shadow-DOM picker (filter input, last-used cached per host). A third command \`pass-fill-identity\` does **everything in one keystroke** — login (username/password) + profile + credit-card. It pre-scans the page and only invokes a picker for categories actually present; login uses the exact same path as the standalone \`pass-fill\` — host-matching a \`pass\` entry with the same eTLD+1 matcher and the shared \`fillLoginForm()\` injector, so a 2-step username-first login (step 1 with no visible password field) fills just as it does under \`pass-fill\`. Gated on host-match alone (no password-field requirement); \`pickLoginEntry\` returns nothing when the host has no stored login, so non-login pages are untouched. Auto-submit is forced off so it never abandons the address/card fields it just wrote. Example \`profile/personal.gpg\` body — first line is a free-form label, the rest are friendly key:value pairs:
...
- **Color schemes (8 + custom)** — pick one of **Cyberpunk / Midnight / Matrix / Ember / Arctic / Crimson / Toxic / Vapor** from the theme page (\`scripts-manager/theme-injector.html\`). The palettes are vendored from the app-shell source of truth in \`lib/color-schemes.js\`; the choice recolors zpwrchrome's OWN pages (popup + every dashboard) at runtime via \`lib/ui-scheme.js\` (overriding each page's CSS \`:root\` default through \`documentElement.style\`, persisted under \`chrome.storage.local["ui.scheme"]\`, broadcast live over \`storage.onChanged\`) AND sets the page-theme injector palette below. One pick, both surfaces. The picker also lists every **custom scheme saved anywhere in the zwire fleet** — \`background.js\` subscribes to the host \`schemes\` topic (the shared saved-scheme library in \`~/.zwire/global.toml\`, mirrored to \`chrome.storage.local["ui.schemes"]\`); picking one writes its resolved \`ui.palette\` and pushes it back to the host so the whole fleet repaints in step. A **custom-scheme editor** (a \`<input type=color>\` swatch per base token; glow/dim/bg tints auto-derive via \`buildCustomScheme\`) plus a saved-scheme library toolbar (name · Save · Update · Delete-all · per-chip delete) let you build/edit/name schemes right in zpwrchrome; every save rewrites the shared library through the host, so a scheme created here shows up on every zwire surface
...
5. \`pass.unlock\` / \`pass.lock\` → \`extensions::gpg_unlock\`
6. \`zcite.save\` → \`extensions::zcite\`
7. anything else → \`process_dispatch\` — the byte-for-byte mirror of upstream \`request/process.go\`: \`configure\` / \`list\` / \`tree\` / \`fetch\` / \`save\` / \`delete\` / \`echo\`
...
| \`gpg_unlock\` | \`pass.unlock\` / \`pass.lock\` | Enter the GPG passphrase from the browser instead of a terminal: decrypts one probe entry with \`--pinentry-mode loopback --passphrase-fd 0\`, which primes gpg-agent for its cache TTL so every later \`--batch\` decrypt works. \`pass.lock\` flushes the cache via \`gpg-connect-agent reloadagent\` |
...
| GPG passphrase entry from the browser (\`pass.unlock\` / \`pass.lock\`) | browserpass (no unlock path — the store must be primed from a terminal) · GUI pinentries (\`pinentry-mac\`, \`pinentry-gnome3\`), which need a desktop session the NM host may not have | The host decrypts a probe entry with \`--pinentry-mode loopback --passphrase-fd 0\`, priming gpg-agent for its cache TTL so the unmodified \`--batch\` decrypt path works afterwards. Passphrase on stdin only — never argv, never disk, never \`chrome.storage\`, excluded from the diag ring, wiped with volatile writes host-side |
...
<head>
  ...
  <meta name="description" content="Chrome extension: UNIX \`pass\` integration, segmented download manager (Chrome default takeover), JetBrains-style tab switcher, fzf history search, Tampermonkey-equivalent userscripts, full-page screenshot, Wappalyzer tech detection, cyberpunk page theme, Turn Off the Lights, JSON viewer, XML viewer, UA switcher, ModHeader-style header manager, find-in-all-tabs. ${total} keyboard commands. Cyberpunk HUD by MenkeTechnologies.">
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* color-scheme strip auto-injected by hud-theme.js (page has no static markup for it) */
    ...
    .scheme-btn { padding: 5px 8px; background: var(--bg-secondary); border: 1px solid var(--border); border-radius: 2px; cursor: pointer; text-align: left; font-family: 'Share Tech Mono', monospace; transition: all 0.2s; }
    .scheme-btn:hover { border-color: var(--cyan); box-shadow: 0 0 10px var(--cyan-dim); }
    .scheme-btn.active { border-color: var(--cyan); border-left: 3px solid var(--cyan); box-shadow: 0 0 12px var(--cyan-glow); }
    .scheme-btn-name { font-size: 12px; font-weight: 600; color: var(--text); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 2px; }
    .scheme-btn-desc { font-size: 10px; color: var(--text-muted); }
    .scheme-btn-preview { display: flex; gap: 4px; margin-top: 5px; flex-wrap: wrap; }
  ...
  <main>
    <section class="card">
      ...
      <div class="features">
        ...
        <div class="feature"><strong>Save to zcite</strong>Page right-click extracts bibliographic metadata (Highwire <code>citation_*</code> / Dublin Core / Open Graph / schema.org JSON-LD) into CSL-JSON; the native host's <code>zcite.save</code> drops it into the <a href="https://github.com/MenkeTechnologies/zcite">zcite</a> reference manager's inbox. The Zotero-Connector role — handed off as a plain file, no link to the proprietary engine.</div>
...
const subsystems = [
  ...
  {
    name: "ModHeader",
    files: ["scripts-manager/modheader.html", "scripts-manager/modheader.js", "scripts-manager/modheader.css"],
    role: "Multi-profile HTTP header manager. Each profile owns N rules; only the active profile's enabled rules are applied. Rule kinds: request header (set/append/remove), response header (set/append/remove), URL redirect. Backed by chrome.declarativeNetRequest dynamic rules in id range 2000-2999.",
  },
  {
...
<head>
  ...
  <meta name="description" content="zpwrchrome — engineering report. Manifest V3 Chrome extension. ${total} keyboard commands, ${popupCategories.length} popup categories, ${bgKinds.length} message kinds, ${num(totalJsLines)} JS lines, ${num(testCount)} tests. Strykelang HUD by MenkeTechnologies.">
  ...
  <style>
    ...
    .bar-fill { height: 100%; transition: width 1.2s cubic-bezier(.22,1,.36,1); }
    ...
    pre.code { background: var(--bg-secondary); border: 1px solid var(--border); border-left: 3px solid var(--cyan); padding: 10px 14px; border-radius: 2px; font-family: 'Share Tech Mono', monospace; font-size: 11.5px; color: var(--text); overflow-x: auto; line-height: 1.55; margin: 8px 0; }
  ...
  <div class="app tutorial-app" id="reportApp">
    ...
    <header class="tutorial-header">
      <div class="tutorial-header-inner">
        <div>
          <h1 class="tutorial-brand">// ZPWRCHROME &mdash; ENGINEERING REPORT</h1>
          <nav class="tutorial-crumbs" aria-label="Breadcrumb">
            <span class="current">Engineering Report</span>
            <span class="sep">/</span>
            <a href="index.html">Landing</a>
            <span class="sep">/</span>
            <a href="https://github.com/MenkeTechnologies/zpwrchrome" target="_blank" rel="noopener noreferrer">GitHub</a>
          </nav>
    ...
    <p class="tutorial-subtitle">Every JS surface in the repo, by role. Pure helpers in <code>lib/util.js</code> + <code>lib/fzf.js</code> carry zero <code>chrome.*</code> references so they unit-test headless and so <code>scripts/build-modal.mjs</code> can inline them into the content-script modal between <code>UTIL_INLINE_START/END</code> and <code>FZF_INLINE_START/END</code> markers.</p>
    <div class="subsystems">${subsystems.map(subsystemCard).join("")}
    </div>

    <h2 class="tutorial-title"><span class="step-hash">#</span>TOP FILES BY SIZE</h2>
    <p class="tutorial-subtitle">Sorted descending. <code>modal/content.template.js</code> is the source; <code>modal/content.js</code> is generated (fonts + fzf + util inlined) and excluded from this table.</p>
...
${topFiles.slice(0, 15).map(fileTableRow).join("\n")}
    ...
    <div class="arch-wrap">
      <svg class="arch" viewBox="0 0 1080 660" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="zpwrchrome process and data flow architecture">
        ...
        <rect class="box host" x="780" y="164" width="262" height="92" />
        <text class="lbl"  x="792" y="184">Extension actions</text>
        <text class="sub"  x="792" y="200">dl.{add,list,pause,resume,cancel}</text>
        <text class="sub"  x="792" y="214">dl.{restart,remove,clear,open*,write*}</text>
        <text class="sub"  x="792" y="228">otp · search · pass.{unlock,lock}</text>
        <text class="sub"  x="792" y="242">run.spawn · host.* · zcite.save</text>
      ...
      <p class="tutorial-subtitle" style="margin-top:6px;">
   ...
   onReplaced                                                              <span class="a">.local</span>
    ...
    <p class="tutorial-subtitle"><strong>${bgKinds.length}</strong> message kinds handled by the service worker. Popup sends <strong>${popupKinds.length}</strong> kinds; modal sends <strong>${modalKinds.length}</strong>. <code>tests/protocol.test.js</code> guards against orphans on either side.</p>
    <ul class="kinds">
${bgKinds.map((k) => kindCard(k, messageRoles[k] || "(handler in background.js)")).join("")}
    </ul>

    <h2 class="tutorial-title"><span class="step-hash">$</span>KEYBOARD COMMANDS</h2>
    <p class="tutorial-subtitle">Chrome MV3 caps commands with default-suggested keys at <strong>4</strong>; everything else is user-bound at <code>chrome://extensions/shortcuts</code>. Currently <strong>${withKey.length}</strong> default-keyed (<code>${withKey.map(([n, v]) => `${n}=${v.suggested_key?.mac || v.suggested_key?.default}`).join("</code>, <code>")}</code>), <strong>${userBound}</strong> user-bound, <strong>${total}</strong> total.</p>
    <div class="stat-grid">
...
${popupCategories.map((c) => `      <li><span class="label">${reportEsc(c.label)}</span><span class="key">${reportEsc(c.key)}</span></li>`).join("\n")}
    ...
    <h2 class="tutorial-title"><span class="step-hash">^</span>HISTORY &mdash; FRECENCY FORMULA</h2>
  ...
  <span class="k">export function</span> <span class="a">frecencyScore</span>(item, nowMs = Date.now()) {
    ...
    <h2 class="tutorial-title"><span class="step-hash">!</span>PERMISSIONS</h2>
    <p class="tutorial-subtitle"><strong>${permissions.length}</strong> declared. <code>tests/static.test.js</code> enforces every declared permission is actually used in <code>background.js</code> or <code>popup.js</code> &mdash; no dead permissions.</p>
    <div>
...
${(manifest.optional_permissions || []).map((p) => `      <span class="perm-pill opt" title="optional_permissions — not requested at install">${reportEsc(p)}</span>`).join("\n")}
    ...
    <h2 class="tutorial-title"><span class="step-hash">*</span>TESTS</h2>
    <p class="tutorial-subtitle">Plain <code>node:test</code>. Zero dependencies. <strong>${testCount}</strong> cases across <strong>${testFiles.length}</strong> files in <strong>~1s</strong>. CI matrix: Node 20 + 22 on ubuntu-latest.</p>
    <table class="file-table">
...
${designDecisions.map(([h, p]) => `      <div class="card"><h3>${reportEsc(h)}</h3><p>${p}</p></div>`).join("\n")}
    ...
    <footer style="text-align:center;padding:2rem;font-size:10px;color:var(--text-muted);font-family:'Orbitron',sans-serif;letter-spacing:2px;">
      ZPWRCHROME v${version} &middot; ${total} COMMANDS &middot; ${testCount} TESTS &middot; ${num(totalJsLines)} JS LINES &middot; MENKETECHNOLOGIES &middot; <a href="https://github.com/MenkeTechnologies/zpwrchrome" style="color:var(--cyan);text-decoration:none;">github.com/MenkeTechnologies/zpwrchrome</a> &middot; regenerate via <code>scripts/gen.sh</code>
    </footer>
  </div>
