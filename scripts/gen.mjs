...
- **Segmented download manager** — same Rust host vendors a multi-connection downloader (HEAD probe → N parallel \`Range\` segments, default 6, pre-allocated dest file). Cookie + User-Agent forwarded from \`chrome.cookies.getAll\` so logged-in downloads work; transient errors retry with 200 ms × 3ⁿ backoff and resume via \`Range\` from the segment-local offset. A truncated response — a CDN closing the connection before \`Content-Length\` bytes arrive — is detected (premature EOF on an incomplete segment is treated as resumable, not success) and a final byte-count gate refuses to stamp a job \`done\` when fewer than \`Content-Length\` bytes landed on disk, so a partial file never reports as complete; forward progress on a resume doesn't count against the retry budget so a repeatedly-truncating server still finishes. Per-row \`restart\` re-downloads from byte zero (discards the partial/old file, respawns a fresh worker — distinct from \`resume\`, which continues from the current offset). Queue mirrored to \`chrome.storage.local\` so the UI paints instantly across service-worker restarts. Right-click \`Download with zpwrchrome\` on links / images / video / audio; \`dl-paste-url\` reads the clipboard via injected \`navigator.clipboard.readText\`. Live queue UI at \`scripts-manager/downloads.html\` subscribes to host push events. Filename collisions auto-rename \`foo.zip\` → \`foo (1).zip\`. Pure-Rust, vendorable TLS (\`ureq\`+rustls), no \`aria2\` or other runtime binary
