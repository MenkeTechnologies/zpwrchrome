...
test("expandFieldValue: name composed from given + additional + family", () => {
  assert.equal(expandFieldValue("name", {
    "given-name": "Jacob", "additional-name": "P", "family-name": "Menke",
  }), "Jacob P Menke");
  assert.equal(expandFieldValue("name", { "given-name": "Jacob", "family-name": "Menke" }),
                                          "Jacob Menke");
});

test("expandFieldValue: given-name / family-name split out of a single name field", () => {
  assert.equal(expandFieldValue("given-name",  { name: "Jacob P Menke" }), "Jacob");
  assert.equal(expandFieldValue("family-name", { name: "Jacob P Menke" }), "Menke");
  // single-word name has no surname
  assert.equal(expandFieldValue("family-name", { name: "Jacob" }), null);
});

test("expandFieldValue: cc-name falls back through name → cc-given/family → given/family", () => {
  assert.equal(expandFieldValue("cc-name", { "name": "Jacob Menke" }), "Jacob Menke");
  assert.equal(expandFieldValue("cc-name", { "cc-given-name": "J", "cc-family-name": "M" }), "J M");
  assert.equal(expandFieldValue("cc-name", { "given-name": "Jacob", "family-name": "Menke" }), "Jacob Menke");
});
...
test("expandFieldValue: realistic profile entry with friendly names round-trips", () => {
  ...
  assert.equal(expandFieldValue("given-name",     friendly), "Jacob");
  assert.equal(expandFieldValue("family-name",    friendly), "Menke");
  assert.equal(expandFieldValue("email",          friendly), "linux.dev25@gmail.com");
  ...
  assert.equal(expandFieldValue("name", friendly), "Jacob Menke");
});
