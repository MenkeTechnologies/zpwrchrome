...
pub fn DetectGpgBinary() -> Result<String, String> {
    ...
    let gpgBinaryPriorityList: &[&str] = &[
        // go:18
        "gpg2",
        "gpg", // go:19
        "/bin/gpg2",
        "/usr/bin/gpg2",
        "/usr/local/bin/gpg2", // go:20
        "/bin/gpg",
        "/usr/bin/gpg",
        "/usr/local/bin/gpg", // go:21
    ];

    for binary in gpgBinaryPriorityList {
        // go:24
...
pub fn GpgEncryptFile(
    ...
    if let Some(parent) = filePath.parent() {
        // go:59 os.MkdirAll(filepath.Dir(filePath), 0755)
        fs::create_dir_all(parent)
            .map_err(|e| format!("Unable to create directory structure: {e}"))?;
        // go:60-62
    }

    let mut gpgOptions: Vec<String> = vec![
        // go:65
        "--encrypt".into(), // go:65
        "--yes".into(),     // go:65
        "--quiet".into(),   // go:65
        "--batch".into(),   // go:65
        "--output".into(),
        filePath.to_string_lossy().into_owned(), // go:65
    ];
