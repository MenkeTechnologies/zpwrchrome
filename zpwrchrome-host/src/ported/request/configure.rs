...
pub fn configure(request: &request) {
    ...
    for store in request.Settings.Stores.values() {
        ...
        responseData
            .StoreSettings
            .insert(store.ID.clone(), raw_string); // go:63
        ...
        if let Some(e) = store_err {
            ...
            response::SendErrorAndExit(
                ...
                Some(response::params_of(&[
                    (
                        errors::field::MESSAGE,
                        "Unable to read .browserpass.json of the password store",
                    ),
                    (errors::field::ACTION, "configure"),
                    (errors::field::ERROR, &e),
                    (errors::field::STORE_ID, &store.ID),
    ...
    if request.Settings.Stores.is_empty() {
        // go:93 len(...) == 0
        let possibleDefaultStorePath = match getDefaultPasswordStorePath() {
            // go:94
            Ok(p) => p,
            Err(e) => {
                // go:95-104
        ...
        let normalized = match normalizePasswordStorePath(&possiblePathStr) {
            // go:106
            Ok(p) => p,
            Err(e) => {
                // go:107-121
        ...
        let default_raw = match readDefaultSettings(&normalized) {
            // go:127
            Ok(s) => Some(s),
            Err(e) => {
...
fn emitUnreadableDefault(store_path: &str, e: &str) -> ! {
    ...
    response::SendErrorAndExit(
        ...
        Some(response::params_of(&[
            (
                errors::field::MESSAGE,
                "Unable to read .browserpass.json of the default password store",
            ),
            (errors::field::ACTION, "configure"),
            (errors::field::ERROR, e),
...
fn dirs_home() -> Result<String, io::Error> {
    std::env::var("HOME").map_err(|_| io::Error::new(io::ErrorKind::NotFound, "$HOME not set"))
}
