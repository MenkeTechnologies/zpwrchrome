...
pub fn deleteFile(request: &request) {
    ...
    if let Err(e) = fs::remove_file(&filePath) {                                // go:66 os.Remove(filePath)
        eprintln!("Unable to delete the password file: {e}");                   // go:68
        response::SendErrorAndExit(                                             // go:69-81
    ...
    loop {                                                                       // go:86 for { ... }
        ...
        if dir == normalizedStorePath {                                         // go:87 if parentDir == store.Path
            break;                                                               // go:88
        }

        let isEmpty = match helpers::IsDirectoryEmpty(&dir) {                   // go:91 helpers.IsDirectoryEmpty(parentDir)
            Ok(b) => b,
            Err(e) => {                                                         // go:92-107
                eprintln!("Unable to determine if directory is empty and can be deleted: {e}"); // go:93
                response::SendErrorAndExit(                                     // go:94-107
        ...
        if let Err(e) = fs::remove_dir(&dir) {                                  // go:115 os.Remove(parentDir)
            eprintln!("Unable to delete the empty directory: {e}");              // go:117
            response::SendErrorAndExit(                                         // go:118-131
