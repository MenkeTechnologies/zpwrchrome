...
pub fn fetchDecryptedContents(request: &request) {
    ...
    if !request.File.ends_with(".gpg") {                                        // go:16 strings.HasSuffix(request.File, ".gpg")
        eprintln!(
    ...
    let store = match request.Settings.Stores.get(&request.StoreID) {           // go:28
        Some(s) => s.clone(),
        None => {                                                               // go:29-41
            eprintln!(
            ...
            response::SendErrorAndExit(                                         // go:34-41
    ...
    let normalizedStorePath = match normalizePasswordStorePath(&store.Path) {   // go:44
        Ok(p) => p,
        Err(e) => {                                                             // go:45-59
            eprintln!(
            ...
            response::SendErrorAndExit(                                         // go:50-59
    ...
    if !request.Settings.GpgPath.is_empty() || !store.Settings.GpgPath.is_empty() {  // go:65
        if !request.Settings.GpgPath.is_empty() {                               // go:66
            gpgPath = request.Settings.GpgPath.clone();                         // go:67
        } else {                                                                 // go:68
            gpgPath = store.Settings.GpgPath.clone();                           // go:69
        }
        if let Err(e) = helpers::ValidateGpgBinary(&gpgPath) {                  // go:71
            eprintln!("The provided gpg binary path '{gpgPath}' is invalid: {e}"); // go:72-75
            response::SendErrorAndExit(                                         // go:76-84
        ...
        match helpers::DetectGpgBinary() {                                       // go:88
            Ok(p) => gpgPath = p,
            Err(e) => {                                                          // go:89-97
                eprintln!("Unable to detect the location of the gpg binary: {e}"); // go:90
                response::SendErrorAndExit(                                      // go:91-97
    ...
    match helpers::GpgDecryptFile(&file_path, &gpgPath) {                       // go:101
        Ok(contents) => {                                                        // go:102
        ...
        Err(e) => {                                                              // go:102-117
            ...
            response::SendErrorAndExit(                                         // go:107-117
                errors::Code::UnableToDecryptPasswordFile,
                Some(response::params_of(&[
                    (errors::field::MESSAGE,    "Unable to decrypt the password file"),
                    (errors::field::ACTION,     "fetch"),
                    (errors::field::ERROR,      &e),
                    (errors::field::FILE,       &request.File),
                    (errors::field::STORE_ID,   &store.ID),
