...
pub fn Process() {
    let requestLength = match parseRequestLength(io::stdin().lock()) {
        // go:39 parseRequestLength(os.Stdin)
        Ok(n) => n,
        Err(e) => {
            // go:40-49
    ...
    let request = match parseRequest(requestLength, io::stdin().lock()) {
        // go:52 parseRequest(requestLength, os.Stdin)
        Ok(r) => r,
        Err(e) => {
            // go:53-62
    ...
    match req.Action.as_str() {
        ...
        "echo" => {
            // go:79
            response::SendRaw(&req.EchoResponse.clone().unwrap_or(Value::Null));
            // go:80
        }
        other => {
            // go:81-89
