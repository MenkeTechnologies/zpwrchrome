...
pub fn saveEncryptedContents(request: &request) {
    ...
    if !request.File.ends_with(".gpg") {                                        // go:16
        eprintln!(
            "The requested password file '{}' does not have the expected '.gpg' extension",
            request.File
        );
        response::SendErrorAndExit(                                             // go:18-25
    ...
    if request.Contents.is_empty() {                                            // go:28
        eprintln!("The entry contents is missing");                             // go:29
        response::SendErrorAndExit(                                             // go:30-36
    ...
    let store = match request.Settings.Stores.get(&request.StoreID) {           // go:39
        Some(s) => s.clone(),
        None => {                                                               // go:40-52
            eprintln!(
            ...
            response::SendErrorAndExit(                                         // go:45-52
    ...
    let normalizedStorePath = match normalizePasswordStorePath(&store.Path) {   // go:55
        Ok(p) => p,
        Err(e) => {                                                             // go:56-70
            eprintln!(
            ...
            response::SendErrorAndExit(                                         // go:61-70
    ...
    if !request.Settings.GpgPath.is_empty() || !store.Settings.GpgPath.is_empty() { // go:76
        if !request.Settings.GpgPath.is_empty() {                               // go:77
            gpgPath = request.Settings.GpgPath.clone();                         // go:78
        } else {                                                                 // go:79
            gpgPath = store.Settings.GpgPath.clone();                           // go:80
        }
        if let Err(e) = helpers::ValidateGpgBinary(&gpgPath) {                  // go:82
            eprintln!("The provided gpg binary path '{gpgPath}' is invalid: {e}"); // go:83-86
            response::SendErrorAndExit(                                         // go:87-95
        ...
        match helpers::DetectGpgBinary() {                                       // go:99
            Ok(p) => gpgPath = p,
            Err(e) => {                                                          // go:100-108
                eprintln!("Unable to detect the location of the gpg binary: {e}"); // go:101
                response::SendErrorAndExit(                                      // go:102-108
    ...
    let recipients = match helpers::DetectGpgRecipients(&filePath) {            // go:114
        Ok(r) => r,
        Err(e) => {                                                             // go:115-129
            eprintln!("Unable to determine recipients for the gpg encryption: {e}"); // go:116
            response::SendErrorAndExit(                                         // go:117-129
    ...
    if let Err(e) = helpers::GpgEncryptFile(&filePath, &request.Contents, &recipients, &gpgPath) { // go:132
        eprintln!(
            "Unable to encrypt the password file '{}' in the password store '{:?}': {}",
            request.File, store, e
        );
        response::SendErrorAndExit(                                             // go:137-149
