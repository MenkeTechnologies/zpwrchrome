                ...
                if method == "GET" {
                    if let Some(range) = range_header {
                        if let Some(rest) = range.strip_prefix("bytes=") {
                            ...
                            let start: usize =
                                parts.first().and_then(|x| x.parse().ok()).unwrap_or(0);
                            let end: usize = parts
                                .get(1)
...
fn wait_for_done(state_dir: &PathBuf, gid: u64, timeout: Duration) -> Value {
    ...
    loop {
        if start.elapsed() > timeout {
            panic!(
                "download did not finish within {:?}, state path={path:?}",
                timeout
            );
        }
        if path.exists() {
...
fn dl_list_returns_every_gid_under_cache_dir() {
    ...
    for gid in [1u64, 2u64] {
        ...
        fs::write(
            cache.join(format!("gid_{gid:06}.json")),
...
fn dl_cancel_marks_state_cancelled() {
    ...
    assert_eq!(resp["status"], "ok");
    let v: Value =
        serde_json::from_str(&fs::read_to_string(cache.join("gid_000009.json")).unwrap()).unwrap();
...
fn dl_clear_scope_done_removes_only_done_state_files() {
    ...
    assert!(
        dlroot.join("file1.bin").exists(),
        "deleteFromDisk=false → dest must remain"
    );
    let _ = fs::remove_dir_all(&cache);
    let _ = fs::remove_dir_all(&dlroot);
}
...
fn dl_clear_scope_failed_removes_failed_and_cancelled() {
    ...
    let gids: Vec<u64> = resp["data"]["cleared"]
        .as_array()
        .unwrap()
        .iter()
...
fn spawn_worker_detaches_worker_from_inherited_fds() {
    ...
    let block = src
        ...
        .nth(1)
        .expect("spawn_worker missing");
    let head = &block[..block.find("\nfn ").unwrap_or(block.len())];
    assert!(
        head.contains("setsid"),
        "spawn_worker must call libc::setsid()"
    );
    assert!(
        head.contains("libc::close"),
        "spawn_worker must close inherited FDs"
    );
    assert!(
        head.contains("pre_exec"),
        "spawn_worker must use pre_exec hook"
    );
}
...
fn host_ignores_chrome_extension_origin_in_argv() {
    ...
    assert!(
        resp_body.contains("\"status\":\"ok\""),
        "unexpected body: {resp_body}"
    );
}
...
fn host_rejects_truly_unknown_argv() {
    ...
    assert!(
        !out.status.success(),
        "host should exit non-zero on unknown args"
    );
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        stderr.contains("unknown argument"),
        "stderr should mention unknown arg: {stderr}"
    );
}
...
fn expand_home_resolves_tilde_against_home_env() {
    ...
    assert_eq!(
        expand_home("~/Downloads").to_string_lossy(),
        "/tmp/zp-expand-home/Downloads"
    );
...
fn dl_add_with_tilde_dir_expands_to_home_in_state() {
    ...
    assert!(
        dest.starts_with(&*home.to_string_lossy()),
        "expected expanded dest, got {dest}"
    );
    assert!(
        !dest.contains("~"),
        "dest must not contain literal ~: {dest}"
    );
    let _ = fs::remove_dir_all(&cache);
...
fn dl_list_emits_dest_exists_true_for_existing_files_false_for_missing() {
    ...
    assert_eq!(
        real["dest_exists"], true,
        "existing file must report dest_exists=true"
    );
    assert_eq!(
        ghost["dest_exists"], false,
        "missing file must report dest_exists=false"
    );
    let _ = fs::remove_dir_all(&cache);
}
...
fn dl_open_dir_refuses_to_reveal_a_path_that_does_not_exist() {
    ...
    assert_eq!(
        resp["status"], "error",
        "dl.openDir on missing path must error, not silently create"
    );
...
fn dl_resume_leaves_old_worker_alone_when_pid_is_alive() {
    ...
    assert_eq!(
        job["status"], "pending",
        "no new worker, status stays pending until live worker picks up"
    );
    assert_eq!(
        job["worker_pid"], my_pid,
        "live worker_pid must NOT be overwritten"
    );
    let _ = fs::remove_dir_all(&cache);
}
...
fn parse_content_disposition_filename_handles_all_three_forms() {
    ...
    assert_eq!(
        p("attachment; filename=\"True Samples.zip\""),
        Some("True Samples.zip".into())
    );
    assert_eq!(
        p("attachment; filename=plain.zip"),
        Some("plain.zip".into())
    );
    assert_eq!(
        p("attachment; filename*=UTF-8''True%20Samples.zip"),
        Some("True Samples.zip".into())
    );
...
fn guess_filename_rejects_query_garbage_so_worker_can_rename_later() {
    ...
    let bad =
        "https://cdn.example.com/J6bpmRyOJonT3VoXnDag%3D%3D&limit=0&content_type=application%2Fzip";
    assert_eq!(
        guess_filename(bad),
        None,
...
fn apply_naming_mask_substitutes_all_tokens() {
    ...
    let r = apply_naming_mask(
        "*host*-*name*.*ext*",
        "song.mp3",
        "https://music.example/album/song.mp3",
    );
    ...
    assert_eq!(
        apply_naming_mask("*nope*-*name*.*ext*", "a.b", "https://x"),
        "*nope*-a.b"
    );
}
...
fn dl_write_file_uses_unique_dest_path_on_conflict() {
    ...
    assert_ne!(
        dest,
        dst_dir.join("a.bin").to_string_lossy(),
...
fn dl_write_file_chunk_rejects_path_traversal_session_ids() {
    ...
    assert!(
        part.exists(),
        "sanitized sessionId must be written under cache"
    );
...
fn probe_headers_exists_with_head_then_range_get_fallback() {
    ...
    assert!(
        worker.contains("probe_headers(&state.url"),
        "run_worker must call probe_headers, not ureq::head() inline"
    );
    assert!(
        !worker.contains("ureq::head"),
...
fn dl_remove_cancels_and_deletes_state_file() {
    ...
    assert!(
        !cache.join("gid_000099.json").exists(),
...
fn generic_stem_for_content_type_picks_friendly_names() {
    ...
    assert_eq!(
        generic_stem_for_content_type("application/octet-stream"),
        "download"
    );
}
...
fn guess_filename_keeps_extensionless_url_stems() {
    ...
    assert_eq!(
        guess_filename("https://x.com/a/photo.jpg").as_deref(),
        Some("photo.jpg")
    );
    // Bare stem with a query string — keep the stem, drop the query.
    assert_eq!(
        guess_filename("https://x.com/image?id=42").as_deref(),
        Some("image")
    );
    assert_eq!(
        guess_filename("https://cdn.x.com/a/b/a1b2c3").as_deref(),
        Some("a1b2c3")
    );
...
fn dl_uses_generic_typed_name_when_url_is_opaque() {
    ...
    assert!(
        initial.contains("download-"),
        "precondition: opaque URL → placeholder: {initial}"
    );
